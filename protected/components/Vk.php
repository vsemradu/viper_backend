...
class Vk {
    ...
    private $v = '5.40'; // версия Api VKontakte @link https://vk.com/dev/versions
     ...
     */
    public function getCodeTokenByUrl($url) {
    ...
    public function upload_photo_server($gid = 0, $album_id=0, $files = [], $return_ids = false, $tags = '') {
        ...
        $data_json = $this->api('photos.getUploadServer', array('group_id' => intval($gid), 'album_id'=>intval($album_id)));
        ...
        foreach ($temp[0] as $key => $data) {
            ...
            $files['file' . ($key + 1)] = (class_exists('CURLFile', false)) ? new CURLFile($data) : '@' . $data;
        }
        ...
        $upload_data['album_id'] = intval($album_id);
        $upload_data['caption'] = $tags;

        $response = $this->api('photos.save', $upload_data);
